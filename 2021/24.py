...
ans1 = [None]*14
ans2 = [None]*14
for index, group in enumerate(range(0, len(lines), 18)):
    push = (lines[group+4]) == "div z 1"
    add_x = int(lines[group+5].split()[2])
    add_y = int(lines[group+15].split()[2])

    if push:
        stack.append((index, add_y))
    else:
        other_index, add_y = stack.pop()
        diff = add_x + add_y
        if diff >= 0:
            ans1[other_index] = 9 - diff
            ans1[index] = 9
            ans2[other_index] = 1
            ans2[index] = 1 + diff
        else:
            ans1[other_index] = 9
            ans1[index] = 9 + diff
            ans2[other_index] = 1 - diff
            ans2[index] = 1

print("".join(str(n) for n in ans1))
print("".join(str(n) for n in ans2))
